"""
c69update.exe — Standalone updater for C69Automation.

Hiển thị progress bar UI (tkinter), chạy độc lập ngoài C69Automation.exe.
Đợi main process kết thúc → giải nén → thay exe → khởi động lại.

Usage (được spawn bởi C69Automation.exe):
...
class UpdaterWindow:
    ...
    def _build_ui(self):
        ...
        tk.Label(
            header, text="🚀  C69 Automation", bg=BG_CARD,
    ...
    def set_done(self, success: bool):
        if success:
            ...
            self.status_var.set("Đang khởi động C69Automation...")
    ...
    try:
        ...
        if success:
            ...
            ctypes.windll.user32.MessageBoxW(
                0,
                "Cập nhật thất bại!\nVui lòng tải lại thủ công từ https://cdn.c69.us/QHTDautomation.zip",
                "C69 Updater — Lỗi",
                0x10
            )
...
def main():
    parser = argparse.ArgumentParser(description="C69Automation Updater")
    parser.add_argument("--pid",  type=int, required=True,  help="PID of C69Automation.exe")
    parser.add_argument("--zip",  type=str, required=True,  help="Path to downloaded zip file")
    parser.add_argument("--exe",  type=str, default="C69Automation.exe", help="Exe filename to replace")
