...
GPU_VENDORS_MAC = [
    ...
    "ANGLE (AMD, AMD Radeon Pro 5300M OpenGL Engine)",
]

GPU_VENDORS_LINUX = [
    "ANGLE (Intel, Intel(R) UHD Graphics 630 (CFL GT2) vulkan 1.3.205)",
    "ANGLE (NVIDIA, NVIDIA GeForce GTX 1080/PCIe/SSE2, OpenGL 4.5.0)",
    "ANGLE (AMD, AMD Radeon RX 580 Series (POLARIS10, DRM 3.42.0, LLVM 15.0.7), OpenGL 4.6)",
]
...
def generate_webgl_fingerprint(
    ...
    if phone_os == "iPhone":
        ...
        renderer = random.choice(GPU_VENDORS_ANDROID)
        webgl["37446"] = renderer
        if "Adreno" in renderer:
            webgl["37445"] = "Google Inc. (Qualcomm)"
        else:
            webgl["37445"] = "Google Inc. (ARM)"
    else:
        # Desktop OS GPU selection
        if desktop_os == "Mac OS X" or (desktop_os and "Mac" in desktop_os):
            renderer = random.choice(GPU_VENDORS_MAC)
            webgl["37446"] = renderer
            if "Apple" in renderer:
                webgl["37445"] = "Google Inc. (Apple)"
        ...
        elif desktop_os == "Linux":
            renderer = random.choice(GPU_VENDORS_LINUX)
            webgl["37446"] = renderer
            ...
            # Windows or fallback
            renderer = random.choice(GPU_VENDORS_WINDOWS)
            webgl["37446"] = renderer
            if "NVIDIA" in renderer:
                webgl["37445"] = "Google Inc. (NVIDIA)"
            elif "Intel" in renderer:
