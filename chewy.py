...
class Chewy:
    ...
    def site_login(self,email,password,sock='',order=False,sock_type=''):
        ...
        self.browser.add_header('https://www.chewy.com/')
        
        r = self.browser.open('https://www.chewy.com/', timeout=30)
                                                                          
        r = self.browser.open('https://www.chewy.com/app/rest/user-context', timeout=30)
        
        r = self.browser.open('https://www.chewy.com/app/rest/header', timeout=30)
        
        r = self.browser.open('https://www.chewy.com/app/rest/featureConfig?configKeys=sfw.chatProvider%2Cnma.autoshipFirstTimeMaxSavings%2Cnma.autoshipFirstTimeDiscount', timeout=30)
        
        r = self.browser.open('https://www.chewy.com/app/login', timeout=30)

        all_forms = self.browser.selectForm(r.text)
        # print(all_forms)
        changeform =None
        for form in all_forms:
            if str(form).find('form-login') != -1:
                changeform = form
                break
        if not changeform:
            print('check die: cant find form-login form')
        ...
        self.add_header('https://www.chewy.com/app/login', extraHeader={'content-type':'application/x-www-form-urlencoded','accept-encoding':'gzip, deflate, br','origin':'https://www.chewy.com',})
        i = 0
        while i < 4:
            try:
                r = self.browser.open('https://www.chewy.com/app/login',data=data, timeout=30)
                html = r.text
                linkget = r.url
    ...
    def check(self,email='', password='', username='', sock='',sock_type=''):
        ...
        login = self.site_login(email, password,sock=sock,sock_type=sock_type)
...
def main():
    ...
    ordervr = Chewy()
    ordervr.check('dinhngockhuong@gmail.com','Hanoi123!@#',sock='',sock_type='proxy')
