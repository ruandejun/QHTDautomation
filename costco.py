...
class Costco:
    ...
    def site_login(self,email,password):
        ...
        html = r.text
        r = self.browser.open(r.url)
        html = r.text
        print(r.url)
        setting_re = re.search('var SETTINGS =(.+?)\]\}\}\;', html)
        # print(r.url)
        if setting_re:
            setting_result = setting_re.group(1).strip()+']}}'
            setting_result = json.loads(setting_result)
            # print(setting_result)
            # {'remoteResource': 'https://signin-ui.costco.com/ecomssoui/500/SignIn.html?isTC=0', 'retryLimit': 3, 'trimSpacesInPassword': True, 'api': 'CombinedSigninAndSignup', 'csrf': 'RlpvUmZsYkZER1pweldNR1pqWitXaXZXaWVWRWZpUzZ3YWhJN2t3RjFPeERSczRkNGVzQTFJVktVME5tUGtoRWVoMFFKMXpuaVA5dGVKdStKRjlwK0E9PTsyMDIyLTA5LTI1VDAyOjA5OjE3LjYzNjQ3MTNaO3kyMmwwd01ZUnZIVE1YMzdaeHoxVlE9PTt7Ik9yY2hlc3RyYXRpb25TdGVwIjo0fQ==', 'transId': 'StateProperties=eyJUSUQiOiI0ZTBlZWY4Yi0yMzZiLTQ5M2ItODc3Yi0zYWM4N2FmMTQwYjIifQ', 'pageViewId': '76beaa91-d9a3-4012-b926-9d774e045efe', 'suppressElementCss': False, 'isPageViewIdSentWithHeader': False, 'allowAutoFocusOnPasswordField': True,
            #     'pageMode': 1, 'config': {'showSignupLink': 'True', 'operatingMode': 'Email', 'sendHintOnSignup': 'False', 'forgotPasswordLinkLocation': 'AfterLabel', 'includePasswordRequirements': 'true', 'enableRememberMe': 'True', 'announceVerCompleteMsg': 'True'}, 'hosts': {'tenant': '/e0714dd4-784d-46d6-a278-3e29553483eb/B2C_1A_SSO_WCS_signup_signin_500', 'policy': 'B2C_1A_SSO_WCS_signup_signin_500', 'static': 'https://signin.costco.com/static/'}, 'locale': {'lang': 'en', 'country': 'US'}, 'xhrSettings': {'retryEnabled': True, 'retryMaxAttempts': 3, 'retryDelay': 200, 'retryExponent': 2, 'retryOn': ['error', 'timeout']}}
            
            action_url = 'https://signin.costco.com/e0714dd4-784d-46d6-a278-3e29553483eb/B2C_1A_SSO_WCS_signup_signin_520/SelfAsserted?tx=' + setting_result['transId']+'&p=B2C_1A_SSO_WCS_signup_signin_520'
            ...
            header = {
                ...
                "x-csrf-token": setting_result['csrf'].strip(),
            }

            self.browser.add_header(
                r.url, extraHeader=header, XMLHttpRequest=True)
            # self.browser.header = header
            # self.browser.add_header(r.url, XMLHttpRequest=True, extraHeader={
            #                         'Accept': 'application/json, text/javascript, */*; q=0.01', 'Content-type': 'application/x-www-form-urlencoded; charset=UTF-8', 'X-CSRF-TOKEN': setting_result['csrf']})
            data= {}
            data['request_type'] = 'RESPONSE'
            data['signInName'] = email
            data['password'] = password
            # print(action_url, data)
            # print(self.browser.header)
            self.browser.add_cookies_to_header()
            # print(self.browser.header)
            r = self.browser.open(action_url, data=data)
