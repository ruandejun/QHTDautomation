...
class Petmeds:
    ...
    def site_login(self,email,password):
        r = self.browser.open('https://www.1800petmeds.com/login')
        html_forms = self.browser.selectForm(r.text)
        login_form = False
        for line_form in html_forms:
            # print(line_form)
            if str(line_form).find('login-form') != -1:
                login_form = line_form
                break
        input_form = self.browser.getInputForm(login_form)
        # print(input_form)
        input_form['loginEmail'] = email
        input_form['loginPassword'] = password
    ...
    "Referrer-Policy": "strict-origin-when-cross-origin"
        ...
        self.browser.add_header('https://www.1800petmeds.com/login', extraHeader=extra_headers)
        r = self.browser.open('https://www.1800petmeds.com/on/demandware.store/Sites-1800petmeds-Site/default/Account-Login?rurl=1', input_form)
        html = r.text
        print(html)
        c = open('dien.html', 'w', encoding='utf-8')
        c.write(html)
        if html.find('Invalid login or password') == -1:
